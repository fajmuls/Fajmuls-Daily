...
import { ArrowLeft, Plus, ChevronLeft, ChevronRight, Activity, Clock, Dumbbell, BicepsFlexed } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-300 pb-20 md:pb-0">
      <div className="flex items-center justify-between">
        ...
        <button onClick={handleCreate} className="flex items-center gap-2 px-6 py-3 bg-orange-600 text-white rounded-full font-bold hover:bg-orange-700 transition-all">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-stone-200 pb-6 mb-6 mt-4">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-orange-600 text-white rounded-2xl shadow-lg ring-4 ring-orange-100 shrink-0">
          ...
          <div>
            ...
            <p className="text-stone-500 text-sm font-medium">
              Pantau aktivitas fisikmu secara visual.
            </p>
          </div>
        </div>
      </header>
             ...
             return (
               <div 
                 ...
                 className={cn(
                   ...
                   hasWorkout ? "border-orange-400 bg-orange-50 hover:bg-orange-100" : (isToday ? "border-stone-300" : "border-stone-100 hover:border-stone-300")
                 )}
               >
                  <span className={cn("text-lg font-bold tracking-tight", hasWorkout ? "text-orange-700" : "text-stone-700")}>
                    {format(date, 'd')}
                  </span>
                  {hasWorkout && (
                    <div className="absolute bottom-1 w-full flex justify-center gap-1">
                      {notesForDay.map(n => (
                         <div key={n.id} className="w-1.5 h-1.5 rounded-full bg-orange-500" />
                      ))}
                    </div>
                  )}
               </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
         <div className="bg-orange-600 border border-orange-500 text-white rounded-3xl p-6 shadow-sm">
            <Activity className="w-6 h-6 text-amber-200 mb-2" />
            <p className="text-3xl font-serif font-bold">{weightliftingCount}</p>
            <p className="text-orange-200 text-xs font-bold uppercase tracking-wider mt-1">Angkat Beban</p>
         </div>
         <div className="bg-orange-600 border border-orange-500 text-white rounded-3xl p-6 shadow-sm">
            <Dumbbell className="w-6 h-6 text-emerald-200 mb-2" />
            <p className="text-3xl font-serif font-bold">{gymCount}</p>
            <p className="text-orange-200 text-xs font-bold uppercase tracking-wider mt-1">Gym</p>
         </div>
         <div className="bg-orange-600 border border-orange-500 text-white rounded-3xl p-6 shadow-sm">
            <Activity className="w-6 h-6 text-blue-200 mb-2" />
            <p className="text-3xl font-serif font-bold">{runningCount}</p>
            <p className="text-orange-200 text-xs font-bold uppercase tracking-wider mt-1">Lari</p>
         </div>
         <div className="bg-orange-600 border border-orange-500 text-white rounded-3xl p-6 shadow-sm">
            ...
            <p className="text-orange-200 text-xs font-bold uppercase tracking-wider mt-1">Total (Menit)</p>
         </div>
      </div>
    </div>
  );
